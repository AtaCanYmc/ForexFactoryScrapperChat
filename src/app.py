...
try:
    ...
    logger.exception(f"Failed to import route blueprints: {e}")
...
if register_blueprints is not None:
    try:
        ...
        logger.exception(f"Failed to register route blueprints: {e}")
else:
    logger.debug(
...
try:
    ...
    logger.debug(f"Middleware module not available; skipping middleware registration: {e}")
...
try:
    ...
    logger.debug(f"Error handlers module not available; using default Flask handlers: {e}")
...
def run_app():
    ...
    try:
        ...
        logger.warning("PORT env var is not an integer; falling back to 8080")
        port = 8080
